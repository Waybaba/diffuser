...
class MuJoCoRenderer:
    ...
    def episodes2img(self, episodes, cols=1, path=None):
        ...
        if len(episodes.shape) == 3: 
            ...
            return self.renders(to_np(episodes), dim=(1024, 256), partial=True, qvel=True, render_kwargs=render_kwargs)
        else:
    ...
    def chains2video(self, chains, cols=1, path=None):
        ...
        for diff_t in range(0, chains.shape[1], INTERVAL):
            chain_frames = []
            chain_frames.append(self.episodes2img(None, chains[:,diff_t]))
            # for t in range(T):

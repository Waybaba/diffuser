...
def eval_bc(diffuser):
	### distill
	N_EPISODES = 1 # num of episodes get from ds
	N_FULLROLLOUT = 1 # num of episodes for eval in env
	device = torch.device("cuda")
	to_log = {}
	...
	episodes_full_rollout = safefill_rollout(episodes_full_rollout) # fill to make them equal length, fill 0 for reward
	...
	LOG_SUB_PREFIX = "ds"
	to_log[f"{LOG_PREFIX}/{LOG_SUB_PREFIX}_reward"] = np.mean([each["r"].sum() for each in episodes_ds])
	to_log[f"{LOG_PREFIX}/{LOG_SUB_PREFIX}_length"] = np.mean([len(each["s"]) for each in episodes_full_rollout]) # ! TODO use length
	LOG_SUB_PREFIX = "full_rollout"
	to_log[f"{LOG_PREFIX}/{LOG_SUB_PREFIX}_reward"] = np.mean([each["r"].sum() for each in episodes_full_rollout])
	to_log[f"{LOG_PREFIX}/{LOG_SUB_PREFIX}_length"] = states_full_rollout.shape[1]
